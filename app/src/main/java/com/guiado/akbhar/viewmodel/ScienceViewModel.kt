...
import com.guiado.akbhar.view.FragmentScience
import com.guiado.akbhar.view.WebViewActivity


class ScienceViewModel (internal var activity: Activity,
                        internal val fragmentProfileInfo: FragmentScience) // To show list of user images (Gallery)
    : BaseObservable() {
    ...
    init {
        ...
        query = db.collection("/NEWS/news_arabic/world").whereEqualTo(LANGUAGE_ID, pref).whereEqualTo("newstype", NewsTypeEnum.SCIENCE).orderBy("growZoneNumber", Query.Direction.DESCENDING).limit(10)
        doGetTalents()
    }
