...
import com.guiado.akbhar.view.FragmentFood
import com.guiado.akbhar.view.WebViewActivity


class FoodViewModel (internal var activity: Activity,
                     internal val fragmentProfileInfo: FragmentFood) // To show list of user images (Gallery)
    : BaseObservable() {
    ...
    init {
        ...
        query = db.collection("/NEWS/news_arabic/world").whereEqualTo(LANGUAGE_ID, pref).whereEqualTo("newstype", NewsTypeEnum.FOOD).orderBy("growZoneNumber", Query.Direction.DESCENDING).limit(20)
        doGetTalents()
    }
