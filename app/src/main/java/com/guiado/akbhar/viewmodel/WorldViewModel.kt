...
import com.guiado.akbhar.view.FragmentWorld
import com.guiado.akbhar.view.WebViewActivity


class WorldViewModel (internal var activity: FragmentActivity,
                      internal val fragmentProfileInfo: FragmentWorld) // To show list of user images (Gallery)
    : BaseObservable() {
    ...
    init {
        ...
        query = db.collection("/NEWS/news_arabic/world").whereEqualTo("newstype", NewsTypeEnum.WORLD).orderBy("growZoneNumber", Query.Direction.DESCENDING).limit(10)
        doGetTalents()
    }
