...
import com.guiado.akbhar.view.FragmentPolitics
import com.guiado.akbhar.view.WebViewActivity


class PoliticsViewModel (internal var activity: FragmentActivity,
                         internal val fragmentProfileInfo: FragmentPolitics) // To show list of user images (Gallery)
    : BaseObservable() {
    ...
    init {
        ...
        query = db.collection("/NEWS/news_arabic/world").whereEqualTo(LANGUAGE_ID, pref).whereEqualTo("newstype", NewsTypeEnum.POLITICS).orderBy("growZoneNumber", Query.Direction.DESCENDING).limit(20)
        doGetTalents()
    }
