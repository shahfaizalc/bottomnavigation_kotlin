package com.guiado.racha.views
...
private constructor(internal var builder: Builder, savedInstanceState: Bundle?) {
    ...
    /**
     * Helper function to attempt to get current fragment_sports
     *
     * @return Fragment the current frag to be returned
     */
    //Attempt to used stored current fragment_sports
    ...
    /**
     * Get the number of fragment_sports stacks
     *
     * @return the number of fragment_sports stacks
     */
    val size: Int
        @CheckResult
     ...
     * (Consider using replaceFragment if you need to change the root fragment_sports for some reason)
    ...
    /**
     * Function used to switch to the specified fragment_sports stack
     *
    ...
    fun switchTab(@TabIndex index: Int, transactionOptions: FragNavTransactionOptions? = null) {
        ...
        if (currentStackIndex != index) {
            ...
            if (index == NO_TAB) {
                ...
                //Attempt to reattach previous fragment_sports
    ...
    /**
     * Push a fragment_sports onto the current stack
     *
     * @param fragment           The fragment_sports that is to be pushed
     * @param transactionOptions Transaction options to be displayed
     */
    @JvmOverloads
    ...
    /**
     * Pop the current fragment_sports from the current tab
     *
     * @param transactionOptions Transaction options to be displayed
     */
    ...
    fun popFragments(popDepth: Int, transactionOptions: FragNavTransactionOptions? = null) {
        if (isRootFragment) {
            throw UnsupportedOperationException(
                    "You can not popFragment the rootFragment. If you need to change this fragment_sports, use replaceFragment(fragment_sports)")
        } else if (popDepth < 1) {
        ...
        //If we can't reattach, either pull from the stack, or create a new root fragment_sports
        if (fragment != null) {
            ft.commit()
        } else {
        ...
        // committed to the fragment_sports before we add to the stack
        if (bShouldPush) {
            mFragmentStacks[currentStackIndex].push(fragment)
        }
    ...
    /**
     * Clears the current tab's stack to get to just the bottom Fragment. This will reveal the root fragment_sports
     *
     * @param transactionOptions Transaction options to be displayed
     */
    @JvmOverloads
    ...
    /**
     * Replace the current fragment_sports
     *
     * @param fragment           the fragment_sports to be shown instead
     * @param transactionOptions Transaction options to be displayed
     */
    @JvmOverloads
    fun replaceFragment(fragment: Fragment, transactionOptions: FragNavTransactionOptions? = null) {
        ...
        if (poppingFrag != null) {
            ...
            //overly cautious fragment_sports popFragment
            val fragmentStack = mFragmentStacks[currentStackIndex]
            if (!fragmentStack.isEmpty()) {
                fragmentStack.pop()
            }
    ...
    /**
     * Helper function to make sure that we are starting with a clean slate and to perform our first fragment_sports interaction.
     *
    ...
    /**
     * Helper function to get the root fragment_sports for a given index. This is done by either passing them in the constructor, or dynamically via NavListener.
     *
     * @param index The tab index to get this fragment_sports from
     * @return The root fragment_sports at this index
     ...
     * constructor, or because your RootFragmentListener.getRootFragment(index) isn't returning a fragment_sports for this index.
     */
    @CheckResult
    ...
    private fun getRootFragment(index: Int): Fragment {
        ...
        if (fragment == null) {
            throw IllegalStateException("Either you haven't past in a fragment_sports at this index in your constructor, or you haven't " + "provided a way to create it while via your RootFragmentListener.getRootFragment(index)")
        }
    ...
    /**
     * Will attempt to reattach a previous fragment_sports in the FragmentManager, or return null if not able to.
     *
     * @param ft current fragment_sports transaction
    ...
    /**
     * Attempts to detach any current fragment_sports if it exists, and if none is found, returns.
     *
    ...
    /**
     * Create a unique fragment_sports tag so that we can grab the fragment_sports later from the FragmentManger
     *
     * @param fragment The fragment_sports that we're creating a unique tag for
     * @return a unique tag using the fragment_sports's class name
     */
    @CheckResult
    ...
    /**
     * Private helper function to clear out the fragment_sports manager on initialization. All fragment_sports management should be done via FragNav.
     */
    ...
    /**
     * Setup a fragment_sports transaction with the given option
     *
    ...
    class Builder(internal val mSavedInstanceState: Bundle?, internal val mFragmentManager: FragmentManager, internal val mContainerId: Int) {
        ...
        /**
         * @param rootFragment A single root fragment_sports. This library can still be helpful when managing a single stack of fragments
         */
        ...
        /**
         * @param rootFragments a list of root fragments. root Fragments are the root fragments that exist on any tab structure. If only one fragment_sports is sent in, fragnav will still manage
         * transactions
         */
        fun rootFragments(rootFragments: List<Fragment>): Builder {
            mRootFragments = rootFragments as MutableList<Fragment>
        ...
        /**
         * @param val A listener to be implemented (typically within the main activity) to fragment_sports transactions (including tab switches)
         */
        ...
        fun build(): FragNavController {
            if (mRootFragmentListener == null && mRootFragments == null) {
                throw IndexOutOfBoundsException("Either a root fragment_sports(s) needs to be set, or a fragment_sports listener")
            }
 ...
 * Push a fragment_sports onto the current stack
 *
 * @param fragment_sports The fragment_sports that is to be pushed
 */
/**
 * Pop the current fragment_sports from the current tab
 */
 ...
 * Replace the current fragment_sports
 *
 * @param fragment_sports the fragment_sports to be shown instead
 */
